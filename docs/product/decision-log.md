...
| PD-019 | 2026-08-12 | Personal history must produce a useful longitudinal pattern report, not only participation or baseline percentages. The report may flag repeated gameplay-pattern changes and suggest a check-in, but it may not name a condition, emit a health-risk score, or claim a medical cause. | Show the period and valid records analysed, recent-versus-usual evidence per metric family, the repeated-change rule, possible everyday influences, and a neutral next step. Keep simulated history unmistakable and describe the pipeline precisely: AI perception supplies derived measures; the current longitudinal detector is a transparent statistical research prototype. |
| PD-020 | 2026-08-14 | The hackathon progress report uses a prominent gameplay result of Stable, Declined, or Improving. Successfully loaded incomplete history defaults to Stable for presentation, while unavailable history remains unavailable. | Add a symmetric upper-range prototype rule for Improving, retain the existing downward-only supporter gate, and describe the feature as AI-assisted because on-device landmark AI supplies its measures. Do not claim an LLM or external trend API exists. |
