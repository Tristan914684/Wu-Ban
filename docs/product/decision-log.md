...
| PD-019 | 2026-08-12 | Scored play uses an elderly-first `Now / You / Next` hierarchy with viewing-distance text and simple, persistent controls. | Separate `In frame` from the calibrated start position; keep a visible player-state rail, four-stage cue timing, four gameplay phases, Pause, and Stop; defer narration and volume controls to the paused surface; require at least 24 px live-state labels and 56 px Pause/Resume targets at the laptop viewport. |
| PD-020 | 2026-08-13 | The hackathon exposes a scalable song library with only `茉莉花` playable, then reduces scored play to a video-led, large-icon stage for older adults. | Keep future songs visibly unavailable pending rights review; use the local project-authored Mo Li Hua motion visual; show only player position, current/upcoming cues, Pause/Stop, four phase dots, time, and short feedback during ordinary play; reduced dynamics simplifies cue travel but never stops or replaces the MV. |
